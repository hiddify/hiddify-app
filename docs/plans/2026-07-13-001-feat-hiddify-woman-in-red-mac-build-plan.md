...
**KTD-5 — Let the Makefile drive core compilation.** Do not hand-run gomobile. `make ios-install-deps` + `make ios-prepare` install gomobile/protoc and build the core into the iOS framework via the project's own targets. Hand-rolling the Go build diverges from what upstream CI does.
...
| Go | `1.24` (toolchain `go1.24.0`) | `hiddify-core/go.mod` (branch `v3`) | Needed to build the core |
...
- `make ios-prepare` — runs `get` (`flutter pub get`), `gen` (build_runner codegen), `translate`, and builds `hiddify-core` into the iOS framework (`ios-libs`).
