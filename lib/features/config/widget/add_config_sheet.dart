...
import 'package:hiddify/features/subscription/subscription.dart';
  ...
  void _processContent(
    ...
    if (result.items.length == 1 &&
      ...
      ScaffoldMessenger.of(
        context,
      ).showSnackBar(const SnackBar(content: Text('Config added!')));
      return;
    }
    ...
    unawaited(
      Navigator.push(
        ...
        MaterialPageRoute<void>(
          builder: (_) =>
              SubscriptionPreviewPage.raw(content: trimmed, source: source),
        ),
      ),
    );
  }
...
class QRScannerPage extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return Scaffold(
      appBar: AppBar(title: Text(t.profile.add.scanQr), centerTitle: true),
      body: Stack(
        children: [
          MobileScanner(
            onDetect: (capture) {
          ...
          Positioned(
            ...
            right: 0,
            child: Text(
              t.profile.add.qrScanner.permissionRequest,
