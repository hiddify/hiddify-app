...
class ConfigExtractor {
  ...
  static final RegExp _configPattern = RegExp(
    r'((?:vless|vmess|trojan|ss|ssr|hysteria2?|hy2|tuic|naive\+https|naive\+quic|naive|wg|wireguard):\/\/[^\s\n\r]+)',
    multiLine: true,
    caseSensitive: false,
  );

  // Checks if the given text contains any proxy configuration
  static bool containsConfig(String text) {
    return _configPattern.hasMatch(text);
  }

  // Counts the number of configurations in the given text
  static int countConfigs(String text) {
    return _configPattern.allMatches(text).length;
  }
  ...
  static String detectProtocol(String url) {
    ...
    if (lowercaseUrl.startsWith('naive+https://') ||
        lowercaseUrl.startsWith('naive+quic://') ||
        lowercaseUrl.startsWith('naive://')) {
      return 'naive';
    }
