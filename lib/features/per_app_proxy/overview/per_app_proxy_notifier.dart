...
Future<List<AppInfo>> installedPackagesInfo(
  ...
  return await ref.watch(perAppProxyRepositoryProvider).getInstalledPackages(false).getOrElse((err) {
...
Future<List<AppInfo>> installedPackagesInfoExcludeSystem(
  ...
  return await ref.watch(perAppProxyRepositoryProvider).getInstalledPackages(true).getOrElse((err) {
    // _logger.error("error getting installed packages", err);
    throw err;
  }).run();
}
