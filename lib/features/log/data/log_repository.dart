...
class LogRepositoryImpl
  ...
  Stream<List<LogEntity>> _watchAppLogs() async* {
    ...
    yield await _readAppLogFile(
      file,
    ).then((_) => _appLogBuffer.map(LogParser.parseApp).toList());
    yield* Watcher(
      file.path,
  ...
  Future<List<String>> _readAppLogFile(File file) async {
    if (_appLogFilePosition == 0 && file.lengthSync() == 0)
      return _appLogBuffer;
    final content = await file
        .openRead(_appLogFilePosition)
  ...
  Stream<Either<LogFailure, List<LogEntity>>> watchLogs() {
    ...
    final app$ = _watchAppLogs().handleExceptions((error, stackTrace) {
      loggy.warning("error watching app logs", error, stackTrace);
      return LogFailure.unexpected(error, stackTrace);
    });

    return Rx.combineLatest2(core$, app$, (
      Either<LogFailure, List<LogEntity>> a,
      Either<LogFailure, List<LogEntity>> b,
    ) {
      return a.fold(
        (l) => left(l),
        (coreLogs) => b.fold(
