...
class PlatformSingboxService with InfraLogger implements SingboxService {
  ...
  static const statsChannel = EventChannel(
    "$channelPrefix/stats",
    JSONMethodCodec(),
  );
  static const groupsChannel = EventChannel("$channelPrefix/groups");
  static const activeGroupsChannel = EventChannel(
    "$channelPrefix/active-groups",
  );
  ...
  TaskEither<String, Unit> validateConfigByPath(
    ...
    return TaskEither(() async {
      final message = await methodChannel.invokeMethod<String>("parse_config", {
        "path": path,
        "tempPath": tempPath,
        "debug": debug,
      });
  ...
  TaskEither<String, Unit> start(
    ...
    return TaskEither(() async {
      ...
      await methodChannel.invokeMethod("start", {"path": path, "name": name});
      return right(unit);
    });
  }
  ...
  TaskEither<String, Unit> restart(
    ...
    return TaskEither(() async {
      ...
      await methodChannel.invokeMethod("restart", {"path": path, "name": name});
      return right(unit);
    });
  }
  ...
  Stream<List<SingboxOutboundGroup>> watchGroups() {
    ...
    return groupsChannel.receiveBroadcastStream().map((event) {
  ...
  Stream<List<SingboxOutboundGroup>> watchActiveGroups() {
    ...
    return activeGroupsChannel.receiveBroadcastStream().map((event) {
      if (event case String _) {
        return (jsonDecode(event) as List).map((e) {
          return SingboxOutboundGroup.fromJson(e as Map<String, dynamic>);
        }).toList();
  ...
  TaskEither<String, Unit> selectOutbound(String groupTag, String outboundTag) {
    return TaskEither(() async {
      ...
      await methodChannel.invokeMethod("select_outbound", {
        "groupTag": groupTag,
  ...
  TaskEither<String, Unit> urlTest(String groupTag) {
    return TaskEither(() async {
      await methodChannel.invokeMethod("url_test", {"groupTag": groupTag});
      return right(unit);
    });
  }
  ...
  Stream<List<String>> watchLogs(String path) async* {
    yield* logsChannel.receiveBroadcastStream().map(
  ...
  TaskEither<String, WarpResponse> generateWarpConfig({
    ...
    return TaskEither(() async {
      ...
      final warpConfig = await methodChannel
          .invokeMethod("generate_warp_config", {
